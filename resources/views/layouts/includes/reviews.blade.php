<div class="comments-section">
    <div class="card card-body my-4" id="review-section">
        <div class="comments-area">
            <h4 class="text-center mb-2"><i class="fas fa-comments"></i> {{$book_reviews->count()}} {{__(str_plural('Review', $book_reviews->count())) }}</h4>
    ...
    <div class="card card-body my-4">
        @if(Auth::check())
            <div class="comment-form">
                <h4 class="text-center">{{ __('Leave a review') }}</h4>
                <form action="{{route('book.review', $book->id)}}" method="post">
                    ...
                    <button type="submit" class="btn btn-primary">{{ __('Review') }}</button>
                </form>
            </div>

            @else
            <div><a href="{{url('login')}}">Login here</a> {{ __('For leave a review') }}.</div>
        @endif
    </div>
</div>
